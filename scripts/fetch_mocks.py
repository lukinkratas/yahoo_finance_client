...
async def main() -> None:  # noqa: D103
    ...
    await process_mock(
        client, 'get_quote', kwargs=dict(tickers='META'), file_name='quotes'
    )
    ...
    types = ','.join([f'annual{t}' for t in TYPES['income_statement']])
    await process_mock(
        client,
        'get_timeseries',
        kwargs=dict(ticker='META', types=types),
        file_name='ts_income_statement',
    )

    types = ','.join([f'annual{t}' for t in TYPES['balance_sheet']])
    await process_mock(
        client,
        'get_timeseries',
        kwargs=dict(ticker='META', types=types),
    ...
    types = ','.join([f'annual{t}' for t in TYPES['cash_flow']])
    await process_mock(
        client,
        'get_timeseries',
        kwargs=dict(ticker='META', types=types),
    ...
    await process_mock(
        client, 'get_options', kwargs=dict(ticker='META'), file_name='options'
    )
    await process_mock(
        client, 'get_search', kwargs=dict(tickers='META'), file_name='search'
    )
    await process_mock(
        client,
        'get_recommendations',
    ...
    await process_mock(
        client, 'get_insights', kwargs=dict(ticker='META'), file_name='insights'
    )

    await process_mock(client, 'get_market_summaries', file_name='market_summaries')
    await process_mock(client, 'get_trending', file_name='trending')
    await process_mock(client, 'get_currencies', file_name='currencies')
